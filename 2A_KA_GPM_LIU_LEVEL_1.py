## SCIPT para leer y graficar los datos de NIVEL 1 del sensor GPM-DPR Banda KA (producto 2A)
...
#print(list(f.keys()))
# me interesarian las variables de : KA__NSZ, KA__NSPRECIP.
...
nsp = f['KA__NSPRECIP'][:]
# luego las dimensiones de : KA__LAT, KA__LON, KA__YEAR, KA__MONTH, KA__DAY, KA__MINUTE.
lat = f['KA__LAT'][:]
lon = f['KA__LON'][:]
yy = str(f['KA__YEAR'][0])
mm = str('%02i' % f['KA__MONTH'][0])
dd = str('%02i' % f['KA__DAY'][0])
hr_i = str('%02i' % f['KA__HOUR'][0])
hr_f = str('%02i' % f['KA__HOUR'][len(f['KA__HOUR'])-1])
...
if variable == 'ref':
    plt.title('Level 1 KA_Band reflectivity corrected near surface \n para ' + fecha_i)
if variable == 'rain' :
    plt.title('Level 1 KA_Band near surface rain \n para ' + fecha_i)
    
plt.savefig(salida+'Ka_Band_'+variable+'_'+fecha_f+'.png', dpi = 300, bbox_inches = 'tight')